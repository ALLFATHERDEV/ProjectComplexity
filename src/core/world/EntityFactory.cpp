...
Entity EntityFactory::createPlayer(Vec2f position) const {
    ...
    m_Components.m_Positions.add(player, { position });
    m_Components.m_Velocities.add(player, { 0.0f, 0.0f});
    m_Components.m_Inputs.add(player, { 250 });
    m_Components.m_CharacterStates.add(player, {CharacterState::IDLE, Direction::DOWN} );
    m_Components.m_Collisions.add(player, { SDL_FRect(4.0f, 8.0f, 24.0f, 24.0f), true, false });
    ...
    m_Components.m_AnimationControllers.add(player, controller);

    InventoryComponent inventory;
    inventory.inventory.create(10, 4);
    m_Components.m_Inventories.add(player, inventory);
...
Entity EntityFactory::createCraftingMachine(Vec2f position, Sprite sprite, const MachineDefinition& machineDefinition) const {
    ...
    m_Components.m_Positions.add(machine, { position });
    m_Components.m_Sprites.add(machine, { sprite, 0, width, height });
    m_Components.m_Collisions.add(machine, { SDL_FRect(0.0f, 0.0f, width, height), true, false });
    m_Components.m_MachineEntities.add(machine, { machineDefinition.uniqueName });
    ...
    m_Components.m_MachineInventories.add(machine, inventory);
    ...
    m_Components.m_CraftingMachines.add(machine, crafting);
    ...
    if (!machineDefinition.fluidPorts.empty()) {
        ...
        m_Components.m_MachineFluids.add(machine, machineFluid);
        createMachineFluidPorts(m_EntityManager, m_Components.m_Positions, m_Components.m_FluidPorts, m_Components.m_MachineFluidPortLinks, machine, position, machineDefinition.fluidPorts);
    ...
    m_Components.m_Interactions.add(machine, interaction);
...
Entity EntityFactory::createMiner(Vec2f position, Sprite sprite, const MinerMachineDefinition& machineDefinition) const {
    ...
    m_Components.m_Positions.add(miner, { position });
    m_Components.m_Sprites.add(miner, { sprite, 0, width, height });
    m_Components.m_Collisions.add(miner, { SDL_FRect(0.0f, 0.0f, width, height), true, false });
    m_Components.m_MachineEntities.add(miner, { machineDefinition.uniqueName });

    MachineInventoryComponent inventory;
    inventory.fuelInventory.create(machineDefinition.fuelWidth, machineDefinition.fuelHeight);
    inventory.outputInventory.create(1, 1);
    m_Components.m_MachineInventories.add(miner, inventory);
...
Entity EntityFactory::createFluidTankMachine(Vec2f position, Sprite sprite, const FluidTankMachineDefinition& machineDefinition) const {
    ...
    m_Components.m_Positions.add(tank, { position });
    m_Components.m_Sprites.add(tank, { sprite, 0, width, height });
    m_Components.m_Collisions.add(tank, { SDL_FRect(0.0f, 0.0f, width, height), true, false });
    m_Components.m_MachineEntities.add(tank, { machineDefinition.uniqueName });
    m_Components.m_FluidTanks.add(tank, { {}, machineDefinition.capacity });

    InteractionComponent interaction;
    interaction.interactionName = machineDefinition.displayName.empty() ? "Fluid Tank" : machineDefinition.displayName;
    interaction.interactionBounds = {-8.0f, -8.0f, width + 16.0f, height + 16.0f};
    m_Components.m_Interactions.add(tank, interaction);
...
Entity EntityFactory::createFluidPumpMachine(Vec2f position,
    ...
    m_Components.m_Positions.add(pump, { position });
    m_Components.m_Sprites.add(pump, { sprite, 0, width, height });
    m_Components.m_Collisions.add(pump, { SDL_FRect(0.0f, 0.0f, width, height), true, false });
    m_Components.m_MachineEntities.add(pump, { machineDefinition.uniqueName });
    m_Components.m_FluidPumps.add(pump, { outputFluid, machineDefinition.outputPerSecond });
    m_Components.m_FluidPorts.add(pump, { FluidPortType::OUTPUT, outputDirection, machineDefinition.outputPerSecond });

    InteractionComponent interaction;
    interaction.interactionName = machineDefinition.displayName.empty() ? "Fluid Pump" : machineDefinition.displayName;
    interaction.interactionBounds = {-8.0f, -8.0f, width + 16.0f, height + 16.0f};
    m_Components.m_Interactions.add(pump, interaction);
...
Entity EntityFactory::createConveyorBelt(Vec2f position, const SpriteAtlas& atlas, Direction direction) const {
    ...
    m_Components.m_Positions.add(belt, { position });
    m_Components.m_Sprites.add(belt, { sprite, 0, 0.0f, 0.0f, false, false });
    m_Components.m_ConveyorBelts.add(belt, { direction });
    ...
    m_Components.m_AnimationControllers.add(belt, controller);
...
Entity EntityFactory::createStorageContainer(Vec2f position, Sprite sprite, Vec2i containerInventorySize, Vec2i sizeTiles, bool isBlocking) const {
    ...
    m_Components.m_Positions.add(container, { position });
    m_Components.m_Sprites.add(container, { sprite, 0, width, height });
    m_Components.m_Collisions.add(container, { SDL_FRect(0.0f, 0.0f, width, height), isBlocking, false });

    InventoryComponent inventory;
    inventory.inventory.create(containerInventorySize.x, containerInventorySize.y);
    m_Components.m_Inventories.add(container, inventory);

    InteractionComponent interaction;
    interaction.interactionName = "Storage Container";
    interaction.interactionBounds = {-8.0f, -8.0f, width + 16.0f, height + 16.0f};
    m_Components.m_Interactions.add(container, interaction);
    return container;
}
...
Entity EntityFactory::createFluidPipe(Vec2f position, Sprite sprite, Direction direction) const {
    ...
    m_Components.m_Positions.add(pipe, { position });
    m_Components.m_Sprites.add(pipe, { sprite, 0, 32.0f, 32.0f, false, true });
    ...
    m_Components.m_FluidPipes.add(pipe, pipeComponent);
    return pipe;
}
...
Entity EntityFactory::createFluidTank(Vec2f position, Sprite sprite) const {
    Entity tank = m_EntityManager.createEntity();
    m_Components.m_Positions.add(tank, { position });
    m_Components.m_Sprites.add(tank, { sprite, 0, 32.0f, 32.0f, false, true });
    m_Components.m_FluidTanks.add(tank, {});
    return tank;
}
...
Entity EntityFactory::createFluidPump(Vec2f position, Sprite sprite, Direction outputDirection, const FluidDefinition* outputFluid, float outputPerSecond) const {
    Entity pump = m_EntityManager.createEntity();
    m_Components.m_Positions.add(pump, { position });
    m_Components.m_Sprites.add(pump, { sprite, 0, 32.0f, 32.0f, false, true });
    m_Components.m_FluidPumps.add(pump, { outputFluid, outputPerSecond });
